document.addEventListener("DOMContentLoaded", () => {
  ...
  let tl = gsap.timeline({
    scrollTrigger: {
      ...
      pin: true,
      start: "top top",
      end: window.innerHeight * 6, // the longer the scroll, the slower is the scroll related to ...
      scrub: 5,
      fastScrollEnd: 2000,
    },
  });
  ...
  tl.to(".scene__two", { opacity: 1, duration: 10 })
    .to(".city__floor", {
      keyframes: {
        "0%": { opacity: "1" },
        "24%": { height: "0" },
    ...
    .to(
      ".city__window",
      { backgroundColor: "#565964", duration: 2 },
      "phaseTwo"
    )
    .to(".scene__two", { backgroundColor: "#6e717c", duration: 2 }, "phaseTwo")
    .to(".city__water", {
      keyframes: {
  ...
  tl.to(".scene__three--one", { opacity: 1, duration: 10 })
    ...
    .from(".planes__patch--one", { yPercent: 100, duration: 4 }, "lastPatch")
    .from('.seed', { display: 'none', duration: 3})

    tl.to(".scene__three--two", { display: 'block', duration: 10 })
    ...
    tl.to(".scene__three--three", { display: 'block', duration: 10 })
    tl.to(".scene__three--four", { display: 'block', duration: 10 })
  ...
  plantButton.addEventListener("click", () => {
    ...
    if (currentIndex === 3) {
      plantButton.classList.add('planes__button--disabled');
    }
  });
  

// create an array from the 'seeds' nodeList, and check if they contain the class 'seed--grow'

console.log((document.querySelector('.seed3').classList.contains('seed3--plant')))
